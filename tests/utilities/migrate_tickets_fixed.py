...
This script should be run after both databases are created.
"""
...
def migrate_tickets():
    ...
    try:
        ...
        with portal_engine.connect() as conn:
            portal_tables = conn.execute(text("""
        ...
        if any(t[0] == 'tech_tickets' for t in portal_tables):
            try:
                ...
                for ticket in tech_tickets:
                    ...
                    existing = tickets_session.query(TechTicket).filter(TechTicket.id == ticket.id).first()
                    if not existing:
                        ...
                        tickets_session.add(new_ticket)
        ...
        if any(t[0] == 'maintenance_tickets' for t in portal_tables):
            try:
                ...
                for ticket in maintenance_tickets:
                    # Check if already exists
                    existing = tickets_session.query(MaintenanceTicket).filter(MaintenanceTicket.id == ticket.id).first()
                    if not existing:
                        # Create new ticket in tickets database
