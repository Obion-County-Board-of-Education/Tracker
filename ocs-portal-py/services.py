...
class TicketsService:
    ...
    async def get_buildings(self) -> List[Dict[str, Any]]:
        """Get all buildings from the tickets service"""
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.base_url}/api/buildings")
                response.raise_for_status()
                return response.json()
        except Exception as e:
    ...
    async def get_building_rooms(self, building_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.base_url}/api/buildings/{building_id}/rooms")
                response.raise_for_status()
